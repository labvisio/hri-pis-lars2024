...
class IterativeTracker(object):
    ...
    def __init__(self, args):
        ...
        self.num_positive_affinities = self.args.num_positive_affinities
        self.unmatched = dict()
        self.tracks = list()
        self.tracks_ids = set()
        self.build3D = None
    ...
    def tracking(self, frame_id, camera_list, detections_list, build3D='TopDown'):
        ...
        asso_time = 0
        for camera, detections in zip(camera_list, detections_list):
            n = len(self.tracks)
            m = len(detections)
            if n > 0 and m > 0:
                start = t.time()
                ...
                remain = np.sum(costs_2d > 0, axis=2) > self.num_positive_affinities  # Shelf 10 Campus 14
                ...
                self.unmatched[camera.cid] = {'camera': camera, 'time': frame_id, 'detections': detections}
            else:
                self.unmatched[camera.cid] = {'camera': camera, 'time': frame_id, 'detections': detections}
        ...
        if self.args.init_method == 'GD':
            self.init_target_GD(frame_id)
...
class IterTrack:
    def __init__(self, track_id, time, cameras, poses2d, pose3d, joints_views, args, build3D='SVD'):
        ...
        self.num_positive_affinities = args.num_positive_affinities
    ...
    def get_3dpose(self, time, cameras, Ts, pose_mat, costs3d=None):
        ...
        binary_lists = np.ones((self.joints, len(cameras)*2) ,dtype=np.int32)
    ...
    def cost_to_last(self, camera, time, pose):
        ...
        return total_cost / len(cost2d) if len(cost2d) > self.num_positive_affinities else 0 # Shelf 10 Campus 14
